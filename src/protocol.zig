...
pub const Alert = enum(u8) {
    ...
    pub fn forLocalError(err: anyerror) ?Alert {
        ...
        return switch (err) {
            ...
            => .illegal_parameter,
            error.TlsUnknownCa, error.CertificateIssuerNotFound => .unknown_ca,
...
test "Alert.forLocalError maps local failures to a matching alert" {
    const cases = [_]struct { err: anyerror, alert: Alert }{
        ...
        .{ .err = error.CertificateNotAuthorizedToSign, .alert = .bad_certificate },
        .{ .err = error.CertificatePathLengthExceeded, .alert = .bad_certificate },
        .{ .err = error.CertificateIssuerNotFound, .alert = .unknown_ca },
        .{ .err = error.CertificateExpired, .alert = .certificate_expired },
        .{ .err = error.CertificateFieldHasInvalidLength, .alert = .decode_error },
        .{ .err = error.CertificateFieldHasWrongDataType, .alert = .decode_error },
        .{ .err = error.CertificateNameConstraintViolation, .alert = .bad_certificate },
    };
    for (cases) |c| {
