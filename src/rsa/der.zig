...
pub const Element = struct {
    ...
    /// `reader` is fixed over memory we already hold, so running out of input
    /// is the only way it can fail; it has no transport to report a failure
    /// from. Discharging that keeps `ReadFailed` out of the error set of
    /// everything that parses a certificate.
